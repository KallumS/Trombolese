...
def report(instrument: Trombolese, slide: float) -> None:
    ...
    print(f"Trombolese bore study -- slide extension {slide * 100:.0f} cm")
    print(f"total length {instrument.total_length(0.0, slide):.3f} m, "
    ...
    print()
    header = (f"{'alpha':>6} {'throat':>8} {'trunc':>6} {'f1':>8} {'f2/f1':>7} "
              f"{'regime ratios (f_n / f_1)':<34} {'harmonicity':>12}")
    print(header)
    print("-" * len(header))
    ...
    for alpha in np.linspace(0.0, 1.0, 11):
        ...
        trunc = instrument.truncation_ratio(alpha, slide)
        print(f"{alpha:6.2f} {instrument.throat_radius(alpha) * 1000:7.2f}mm "
              f"{trunc:6.3f} {found.freqs[0]:8.1f} {found.overblow_ratio:7.3f} "
              f"{ratios:<34} {fit.rms_cents:9.0f}c{flag}")
